...
{
    [Activity(Label = "ListadoProductosActivity")]
    public class ListadoProductosActivity : BaseActivity
    {
        private ListView listProductos;
        public static string comercial = "V001";
        private AdapterListadoProductos adapter { get; set; }
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            listProductos = FindViewById<ListView>(Resource.Id.listViewNuevaOfertaFragment);

            //listProductos.AddHeaderView(0).Text = "changed"


            listProductos.Adapter = getAdapterListadoProductos(comercial);
            
            listProductos.ItemClick += OnListItemClick;
            sve = new SearchViewExpandListener(adapter);
            this.SupportActionBar.Show();
            SupportActionBar.Subtitle = "Listado de Productos";

        }
        void OnListItemClick(object sender, AdapterView.ItemClickEventArgs e)
        {

            Producto c = adapter.list[e.Position];
        ...
        public IListAdapter getAdapterListadoProductos(string comercial)
        {
            List<Producto> l = AccesoDatos.listaProductos; //.Where(p => p.salesperson_CodeField == comercial).ToList();

            adapter = new AdapterListadoProductos(this, l);
            return adapter;
        }
